...
public:
  ...
  uint16_t calculate(It begin, It end, It code_loc) noexcept {
    return static_cast<Derived*>(this)->calculate_code_impl(begin, end, code_loc);
  }

  template<typename It>
  bool verify(It begin, It end, It code_loc) noexcept {
...
protected:
  ...
  template<typename It>
  uint16_t calculate_code_impl_(It begin, It end, uint16_t crc = 0xFFFF) noexcept {
    It body_iter = begin;
    while (body_iter != end) {
