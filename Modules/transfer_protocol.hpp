...
public:
  ...
  static constexpr std::size_t header_size =
      sizeof(uint8_t) + sizeof(uint16_t) * 3; // header + size + code + crc
  ...
  [[nodiscard]] bool verify_verification() const noexcept {
    ...
    return va.verify(m_data.begin(), m_data.end(),
                     m_data.begin() + sizeof(uint8_t) + sizeof(uint16_t) * 2);
  }

  explicit operator bool() noexcept { return !m_data.empty(); }
...
public:
  ...
  template <std::input_iterator It> void receive(It begin, It end) {
    ...
    while (true) {
      auto packet_start = std::find(m_receive_buffer.begin(),
                                    m_receive_buffer.end(), packet_t::header);
